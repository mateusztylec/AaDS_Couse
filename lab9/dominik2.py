...
class GraphList:
    ...
    def insertVertex(self, vertex: Vertex):
        self.map_idx[vertex] = self.order()
        self.list_ngh[self.order()] = []
        self.nodes.append(vertex)

    def insertEdge(self, vertex1, vertex2, edge=1):
        id_1 = self.getVertexIdx(vertex1)
        id_2 = self.getVertexIdx(vertex2)
        self.list_ngh[id_1].append((id_2, edge))  # adding with weight of edge
        self.list_ngh[id_2].append((id_1, edge))
        self.list_ngh[id_1] = sorted(set(self.list_ngh[id_1]))
        self.list_ngh[id_2] = sorted(set(self.list_ngh[id_2]))

    def deleteVertex(self, vertex):
        id_vertex = self.getVertexIdx(vertex)
        self.nodes.remove(vertex)  # remove from list
        # reindexing dict:
        new_dict = {}
        for k, v in self.map_idx.items():
            if v == id_vertex:
                continue
            if v > id_vertex:
                new_dict[k] = v - 1
                ...
                new_dict[k] = v
        self.map_idx = new_dict

        # recreating neighbours list:
        new_neigh_list = {}
        for key in self.list_ngh.keys():
            new_values = []  # new list of values for every key
            for value in self.list_ngh[key]:
        ...
        self.list_ngh = new_neigh_list

    def deleteEdge(self, vertex1, vertex2):
        id_1 = self.getVertexIdx(vertex1)
        id_2 = self.getVertexIdx(vertex2)

        for edge in self.list_ngh[id_1]:
            if edge[0] == id_2:
                self.list_ngh[id_1].remove(edge)
                break
        for edge in self.list_ngh[id_2]:
            if edge[0] == id_1:
                self.list_ngh[id_2].remove(edge)
                break
    ...
    def neighbours(self, vertex_idx):
        return self.list_ngh[vertex_idx]
    ...
    def DFS(self, s):
        ...
        while S:
            ...
            if v not in visited:
                visited.append(v)
                if v in self.list_ngh.keys():
                    self.list_ngh[v].reverse()
                    for u in self.list_ngh[v]:
                        S.append(u[0])
        return visited

    def __str__(self):
        return "\n".join([str(k) + " : " + str(v) for k, v in self.list_ngh.items()])
...
class MST:
    ...
    def graph_MST(self, value):
        ...
        for vertex in self.base_graph.nodes:
    ...
    def Prim(self, v):
        ...
        while self.intree[v] == 0:
            current = v
            self.intree[v] = 1
            for neighbour in self.base_graph.list_ngh[v]:
            ...
            min_cost = float("inf")
            for vertex in self.base_graph.list_ngh.keys():
            ...
            for edge in self.base_graph.list_ngh[self.parent[v]]:
                if edge[0] == v:
                    edge_weight = edge[1]
